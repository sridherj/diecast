...
| 7 | Settings.json injection follows gstack's `gstack-settings-hook` pattern: read → defensive JSON parse → mutate in memory → atomic write (`.tmp` + `os.replace`). Idempotent dedup via command-substring match. **Never replace existing hooks**; only append our own / surgically remove our own. | Reference: `<GSTACK_ROOT>/bin/gstack-settings-hook`. Battle-tested. We use stdlib Python (no `bun` dep). |
...
**Linked files:** this plan, `cli/install_hooks.py`, `cli/hook_events.py`, `cli/hook.py`, `pyproject.toml` (`[project.scripts]` entry), reference: `<GSTACK_ROOT>/bin/gstack-settings-hook` (canonical pattern).
...
- **Settings injection pattern (canonical reference):** `<GSTACK_ROOT>/bin/gstack-settings-hook` — atomic `.tmp + rename`, idempotent dedup by command substring, surgical uninstall.
