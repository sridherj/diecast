...
| 4.3 | FR-017 Three-Access-Tiers Parity Moment (hosted in the spike flow) | `sp3_parity_moment/` | 4.1 | Done | Script-patch-driven parity reveal (`appState.parityOpen`, `parity-*` prefix); ink-dark terminal pane (identity exception); same E4 verdict card in both panes; wires the beat into `SCRIPTS.spike`'s reserved slot. **Critical path.** |
| 4.4 | Four-Family Stitch, Slop Gate & Drift Sweep | `sp4_stitch_gates/` | 4.1, 4.2, 4.3 | Done | Stitch all four scripts; **SC-001 + SC-005** met; slop gate + extended drift grep clean; Phase 4 decision summary appended to `decisions-so-far.md`. Verified on a LIVE browser. **Critical path (terminal).** |
...
- 2026-06-12 · **4.3 sp3_parity_moment DONE** — FR-017 three-access-tiers parity beat wired into `SCRIPTS.spike`'s reserved slot (after the verdict beat, before the L3 stop). Reveal is the additive `appState.parityOpen` flag, flipped ONLY by the script's patch — NO sixth op, NO new `drillInto` target (closed 5-op set intact). New `SpikeParity` component (new `parity-*` CSS prefix): an INK-DARK terminal pane (the prototype's one sanctioned identity exception — minimal title bar, IBM Plex Mono, static, no typing animation) rendered ENTIRELY from `goals['CAST-452'].parity` (command + transcript + the `✓` artifact-landing line — nothing typed in markup) with the verdict artifact LANDING in it, beside the normal light canvas/memo pane; both panes render the SAME `EvidenceE4` card resolved via `parity.artifact_id` (a render-time drift guard re-asserts `E4.id === parity.artifact_id`), and the persistent chat rail is the third tier — three tiers, one substrate, one screenful. The exit folds into the next (L3) beat: `parityOpen=false` → the spike canvas is restored with `stageFocus`+receipts intact; reveal is a slide that becomes a fade under `prefers-reduced-motion`. `org.js`/`generate-org.mjs` UNTOUCHED (consumed the 4.1 `parity` block read-only). **Verification — REAL live browser this run** (Claude-in-Chrome reachable; prototype served over `http://localhost`): advanced `SCRIPTS.spike` to the parity beat → split mounts, `parityOpen:true`, both `.ev--e4` cards carry id `E4-CAST-452` with identical answer text, terminal renders 7 transcript lines, chat rail visible, caption from `parity.caption`; **vt-anchor audit via `javascript_tool` = exactly 6 authored names, all unique** (parity layout adds zero); advancing once more EXITS cleanly (split gone, canvas+spine restored, receipts intact). `node --check` clean. **Three-tiers screenshot kept** at `prototype/assets/fr017-parity-three-tiers.png` (a real headless-Chrome render of the committed prototype, CDP-driven to the parity beat — no markup edits). No carry-forward needed: the rendered-pixel items were verified on a live browser this run.
...
- 2026-06-12 · **4.4 sp4_stitch_gates DONE** — terminal stitch + slop gate + drift sweep, verified on a LIVE browser (`http://127.0.0.1:8799`, Claude-in-Chrome). **Stitch:** `scriptKey` set per-goal by family on all four routes (412→feature, 431→debug, 452→spike, 461→data); `SCRIPTS = {feature, debug, spike, data}` complete + CLOSED (F2); no scaffolding flags; **Phase-3 morph demo untouched** (walked CAST-412 live: feature→debug→feature round-trip, family restored, console clean); vt- anchors **6×1**. **Live four-script walk (SC-001):** all four routes walked start-to-finish via `advance()`, **console clean throughout**; spike parity reveal mounts the three-tier split (both panes carry identical `E4-CAST-452`) and exits cleanly; data L3 resolution flips `dataResolved` → E5 re-renders to the reconciled inline-`<svg>` view + one receipt `DEC-CAST-461-03`, ORG unmutated. **SC-005:** the four spine shapes (segment bar / loop band + ↺ / timebox meter / pipeline DAG) confirmed distinct at a glance live. **L3 audit:** exactly one needs-you chip per flow (4/4). **Extended drift grep** (CAST-452/461 · 180ms · 200ms · 1h40m · 8% · finance DB · billing export) CLEAN — every token hit in `data/org.js`/`data/_build`; the only `index.html` hits are CSS motion durations / a parser comment / skeleton-width substrings (non-data). **Slop gate** (checkers not in allowlist → self-assessment vs REAL screenshots): visual not-generic/not-ai-aesthetic PASS on all four; tone PASS with **CF3** (em-dashes in narration + 3 new data strings = standing carry-forward, unified de-em-dash pass deferred). Ink-dark parity terminal AFFIRMED as the sanctioned Decision-7 identity exception (no paper-light fallback). **Phase-5 courtesy:** no Phase-5 sections in `index.html` → zero collision. Phase 4 decision summary appended to `decisions-so-far.md`. Carry-forwards (non-blocking): CF3 de-em-dash pass; eyeball the parity-fade motion + the committed 4-up composite PNG (deferred per owner direction — the plan's sanctioned "verified or carry-forward" path).
