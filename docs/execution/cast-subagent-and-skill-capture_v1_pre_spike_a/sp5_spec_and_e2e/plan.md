...
New spec `docs/specs/cast-subagent-and-skill-capture.collab.md` documents the
contract; registered in `_registry.md`; `cast-spec-checker` lints clean; an
end-to-end manual smoke confirms a real Claude Code session produces the
expected tree + skill chips.

## Dependencies

- **Requires completed:** sub-phases 1-4 (spec reflects shipped behavior).
...
- Cross-spec back-reference in sibling slash-command spec
  (`cast-user-invocation-tracking.collab.md`) noting subagent rows ride the
  same `session_id` field but use a different `source` discriminator.
- Update `cast-hooks.collab.md` (or note in new spec) covering per-event
  matcher contract.

**Out of scope (do NOT do):**

- Code changes (sp1-sp4 should have shipped everything).
- Removing the `system-ops` auto-seed contract from `cast-hooks.collab.md` if
  it's there (sibling owns).
...
| `docs/specs/cast-delegation-contract.collab.md` | Modify | One-line back-reference noting session_id-based hook path. |
| `docs/specs/cast-user-invocation-tracking.collab.md` | Modify | One-line back-reference noting subagent rows use a different `source` discriminator; constants live in `_invocation_sources.py`. |
...
- The parent-resolution SQL keyed on `session_id`.
- The `_invocation_sources.py` constants and their use in `complete()` of each
  service.
...
- Spec maturity: Draft.
- `linked_files` lists all touched files including `_cast_name.py` and
...
→ Delegate: `/cast-spec-checker docs/specs/cast-subagent-and-skill-capture.collab.md`.

Must exit 0. If lint fails, fix the spec (or push back to `/cast-update-spec`)
and re-run.

### Step 5.3: Update `_registry.md`

Add the row (the update-spec skill should do this; if it doesn't, append
manually):
...
- Add a one-line back-reference noting the new session_id-based hook path
  (does not modify any delegation-contract behavior; hook rows don't write
  delegation output files).

Edit `docs/specs/cast-user-invocation-tracking.collab.md` (sibling spec):
- Add a one-line back-reference noting that subagent rows ride the same
  `session_id` field but use a different `source` discriminator
  (`subagent-start` vs `user-prompt`) and that the source constants live in
  `_invocation_sources.py`.

Edit `docs/specs/cast-hooks.collab.md` (sibling spec) if needed:
- Note the per-event matcher extension to `install_hooks.py`. If the new
  spec covers it adequately and `cast-hooks.collab.md` has a "see also"
  section, a back-reference may suffice.

### Step 5.5: End-to-end manual smoke

In a fresh Claude Code session in this project (cast-hook installed via
`cast-hook install`):
...
3. Watch the row tree:
   ...
   - When the slash-command harness dispatches `cast-detailed-plan` as a
     `Task()` subagent, that row appears as child via this plan's
     SubagentStart capture.
   - `cast-detailed-plan`'s auto-trigger of `cast-plan-review` appears as
     grandchild (HTTP-dispatched, sibling-managed).
   - Skill chips appear on whichever cast-* row was most-recent-running when
     each Skill invocation happened.
4. Capture screenshots into
   `goals/cast-subagent-and-skill-capture/notes/e2e-smoke.ai.md` for the
   audit trail.
...
mkdir -p .claude && cp ~/work/somewhere/seeded-third-party-settings.json .claude/settings.json
cast-hook install
cast-hook uninstall
diff <(cat .claude/settings.json) <(cat ~/work/somewhere/seeded-third-party-settings.json)
# Expected: no diff — all 5 cast-hook entries removed; nothing else touched.
```
...
- E2E smoke (Step 5.5): user-invocation root → subagent child → grandchild
  visible in `/runs`; skill chips on appropriate rows.
- Uninstall round-trip (Step 5.6): all 5 hook entries removed; nothing else
  touched.

### Success Criteria

- [ ] `docs/specs/cast-subagent-and-skill-capture.collab.md` exists and lints
      clean.
- [ ] `_registry.md` shows the new row.
- [ ] Cross-spec back-references added to `cast-delegation-contract` and
      `cast-user-invocation-tracking` (and `cast-hooks` if applicable).
...
- **Architecture:** registers via canonical `_registry.md` row; no special
  handling. ✓

## Execution Notes

- **Skill/agent delegation is mandatory** for the spec author/lint loop.
  Don't hand-author the spec — let `/cast-update-spec` produce a
...
- **Screenshot discipline:** capture the L2 chip view AND the L3 detail
  view for at least one row. Show the parent/child/grandchild tree
  expanded.
- **Don't auto-resolve open questions** during spec authoring. If
  `/cast-update-spec` flags a behavior the plan doesn't cover, surface to
