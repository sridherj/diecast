...
Manage Diecast tasks directly via `uv run python -c` commands from the `diecast/` directory.
...
```bash
PYTHONPATH=$HOME/workspace/diecast/cast-server/src uv run python -c "
from cast_server.services.task_service import get_tasks_for_goal
import json
...
```bash
PYTHONPATH=$HOME/workspace/diecast/cast-server/src uv run python -c "
from cast_server.services.task_service import create_task
import json
t = create_task(
    goal_slug='GOAL_SLUG',
    title='TITLE',
...
```bash
PYTHONPATH=$HOME/workspace/diecast/cast-server/src uv run python -c "
from cast_server.services.task_service import update_task
import json
...
```bash
PYTHONPATH=$HOME/workspace/diecast/cast-server/src uv run python -c "
from cast_server.services.task_service import update_task_status
import json
...
```bash
PYTHONPATH=$HOME/workspace/diecast/cast-server/src uv run python -c "
from cast_server.services.task_service import complete_task
import json
t = complete_task(
    TASK_ID,
    actual_time='25m',
