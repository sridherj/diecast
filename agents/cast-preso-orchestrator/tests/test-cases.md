# Test Cases: taskos-preso-orchestrator
...
- Expected: Initializes state.json (version: 2), extracts slide list, enters
  STAGE2_PLANNING, dispatches taskos-preso-what-planner (single child)
...
- Expected: Orchestrator reads manifest, populates state.json stage2.slides with
  8 entries (all pending), dispatches 8 taskos-preso-what-worker children in parallel
...
- Expected: Orchestrator presents escalation to SJ via interactive questions
- Verify: SJ gets clear options (accept/guide/simplify/skip), decision recorded in state.json
...
- Verify: SJ can approve all, flag specific slides, or reject

## TC-8: G3b Open Questions
- Setup: 3 blocking + 2 nice-to-have open questions
- Expected: Orchestrator walks SJ through blocking questions one at a time
...
- Verify: Conservative reconstruction, presented to SJ for confirmation

## TC-11: G3a Gate Rejection + Selective Rework
- Setup: All Stage 3 slides passed, cross-slide passed. At G3a, SJ flags
...
- Verify: rework_count incremented for flagged slides only. SJ's feedback
...
- Setup: All 8 slides approved by checker. At G2, SJ flags 2 slides with
...
- Setup: At G2, SJ chooses Option C (Reject and redo Stage 2).
...
- Expected: Orchestrator treats as corrupt, falls back to reconstruction from
  disk artifacts (§12.5), presents reconstruction summary to SJ before proceeding.
- Verify: New state.json written with version: 2; SJ prompted to confirm before
  any dispatch.
