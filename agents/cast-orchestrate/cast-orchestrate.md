...
```bash
cd "$(cd ~/.claude/skills/diecast && pwd -P)/cast-server" && .venv/bin/python -m cast_server.services.orchestration_service parse-manifest <project_dir>/_manifest.md
```
...
3. Update the manifest:
   ```bash
   cd "$(cd ~/.claude/skills/diecast && pwd -P)/cast-server" && .venv/bin/python -m cast_server.services.orchestration_service update-status <manifest_path> <subphase_id> Done
   ```
...
- Run Python commands with the project venv: `cd "$(cd ~/.claude/skills/diecast && pwd -P)/cast-server" && .venv/bin/python`
