...
| FR-002 | Each L2 assertion on the Goal Card resolves inline markdown the same way, while staying a single contiguous `<li>` with no fragmenting spans. | The "no markdown wrapper" comment is the bug, not the contract. |
...
- **[DEFERRED]** Whether the sibling `_first_sentence` abbreviation-truncation defect should be fixed in the same change or tracked as its own bug. Leaning separate — different root cause, different test, and bundling risks scope creep in a one-file fix.
