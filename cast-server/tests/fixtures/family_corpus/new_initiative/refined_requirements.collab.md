...
| FR-005 | The generated render is cached against the source content hash and regenerated when the source changes, reusing the v2 embedded-source-hash lazy-regeneration mechanism unchanged. | Caching only skips re-rendering identical content; it does not ration generation effort. |
