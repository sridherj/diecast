...
description: >
  Diecast UI e2e: dashboard screen. Drives the dashboard page in Chromium via
  Playwright (delegated to runner.py) — owns goal creation for the harness.
tags:
  - test
  - ui
type: test
...
Drive the `dashboard` page of the Diecast UI in a Chromium browser via Playwright,
...
{
  "contract_version": "2",
  "agent_name": "cast-ui-test-dashboard",
  "task_title": "UI e2e: dashboard screen",
...
Covers **US4 S1, S2, S2b**.

- This child is dispatched **first** by the orchestrator and is responsible for **creating
  the shared `goal_slug`** that subsequent children consume.
- Also creates a separate `ui-test-delete-<unix_ts>` goal to exercise the delete-flow
  assertion. The throwaway goal is removed by the runner; teardown is not this agent's
  responsibility.
- Asserts the three dashboard tabs (active / inactive / completed) swap content without
  full-page reloads, and the create-goal form round-trips to a visible card.
