---
name: cast-ui-test-scratchpad
description: >
  Diecast UI e2e: scratchpad screen. Drives the /scratchpad page in Chromium via
  Playwright (delegated to runner.py) — exercises entry create + delete.
tags:
  - test
  - ui
type: test
...
Drive the `scratchpad` page of the Diecast UI in a Chromium browser via Playwright,
...
{
  "contract_version": "2",
  "agent_name": "cast-ui-test-scratchpad",
  "task_title": "UI e2e: scratchpad screen",
...
- Submits a uniquely-marked scratchpad entry, asserts the new entry is visible in the
  list, then deletes it and asserts the entry is gone.
