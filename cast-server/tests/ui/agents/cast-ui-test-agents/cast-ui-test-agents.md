---
name: cast-ui-test-agents
description: >
  Diecast UI e2e: agents screen. Drives the /agents registry page in Chromium via
  Playwright (delegated to runner.py) and verifies test-agent merge.
tags:
  - test
  - ui
type: test
...
Drive the `agents` page of the Diecast UI in a Chromium browser via Playwright,
...
{
  "contract_version": "2",
  "agent_name": "cast-ui-test-agents",
  "task_title": "UI e2e: agents screen",
...
- Verifies that `/agents` loads, `GET /api/agents` returns a non-empty registry, and
  at least one `cast-ui-test-*` card is visible — proving the `CAST_TEST_AGENTS_DIR`
  registry merge worked.
- Exercises a filter-button toggle and a card-details expansion to confirm interactivity.
