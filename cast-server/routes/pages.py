...
from taskos.config import STATUS_TRANSITIONS, PHASES, PHASE_ARTIFACTS, SCRATCHPAD_PATH, DIGESTS_DIR
from taskos.services import goal_service, task_service
from taskos.services.agent_service import get_all_agents, get_all_runs, get_dashboard_summary, get_escalated_agents
from taskos.services.task_suggestion_service import get_pending_suggestions
...
@router.get("/news")
async def news(request: Request, date: Optional[str] = None):
    """News digest page with date navigation."""
    # Get available digests (sorted descending by date)
    digest_files = sorted(DIGESTS_DIR.glob("*-news-digest.html"), reverse=True)
    digests = []
    for f in digest_files:
        # Parse date from filename: YYYY-MM-DD-news-digest.html
        parts = f.stem.replace("-news-digest", "")
        digests.append({"date": parts, "path": str(f), "filename": f.name})

    if not digests:
        return templates.TemplateResponse(request, "pages/news.html", {
            "active_page": "news",
            "digest_html": None,
            "current_date": None,
            "prev_date": None,
    ...
    # Find current digest
    if date:
        current = next((d for d in digests if d["date"] == date), None)
    else:
        current = digests[0]  # Most recent

    if not current:
        # Requested date not found — show empty state for that date
        return templates.TemplateResponse(request, "pages/news.html", {
            "active_page": "news",
            "digest_html": None,
            "current_date": date,
    ...
    # Read digest HTML
    digest_html = Path(current["path"]).read_text()

    # Compute prev/next dates
    current_idx = next(i for i, d in enumerate(digests) if d["date"] == current["date"])
    prev_date = digests[current_idx + 1]["date"] if current_idx + 1 < len(digests) else None
    next_date = digests[current_idx - 1]["date"] if current_idx > 0 else None
